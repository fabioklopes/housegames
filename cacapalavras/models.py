...
class Participante(models.Model):
    ...
    jogador = models.ForeignKey(Jogador, on_delete=models.CASCADE, related_name='participacoes_cacapalavras')
...
class QuadroJogador(models.Model):
    """Tabuleiro individual de um participante numa rodada.

    Cada jogador tem grid, posições e cores próprias — apenas o nível/tempo
    da rodada é compartilhado entre os participantes.
    """
    ...
    bonus_encontrados = models.JSONField(default=list, blank=True)
    ...
    def completou(self):
        return all(item['encontrada'] for item in self.lista_palavras)

    def contar_encontradas(self):
        return sum(1 for item in self.lista_palavras if item['encontrada'])


class PalavraParaAvaliacao(models.Model):
    """Seleção em linha reta que formou uma palavra não reconhecida (fora da
    lista do jogador e do banco curado). Fica pendente até um administrador
    avaliar e, se aprovada, credita pontos ao jogador que encontrou.
    """
    class Status(models.TextChoices):
    ...
    partida = models.ForeignKey(Partida, on_delete=models.CASCADE, related_name='palavras_para_avaliacao')
    participante = models.ForeignKey(Participante, on_delete=models.CASCADE, related_name='palavras_para_avaliacao')
    rodada = models.ForeignKey(Rodada, on_delete=models.CASCADE, related_name='palavras_para_avaliacao')
    ...
    def __str__(self):
        return f'{self.palavra_ascii} ({self.participante.jogador.apelido}) - {self.status}'
