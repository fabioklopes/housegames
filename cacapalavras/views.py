...
def criar_partida(request):
    if request.method == 'POST':
        ...
        return redirect('cacapalavras_lobby', codigo=partida.codigo)

    return render(request, 'cacapalavras/criar.html')
...
def lobby(request, codigo):
    ...
    if partida.status != Partida.Status.AGUARDANDO:
        return redirect('cacapalavras_jogo', codigo=codigo)

    e_host = _eh_host(request, partida)
    return render(request, 'cacapalavras/lobby.html', {
        'partida': partida,
        'e_host': e_host,
    })
...
def qrcode_imagem(request, codigo):
    partida = get_object_or_404(Partida, codigo=codigo)
    url_entrada = request.build_absolute_uri(reverse('cacapalavras_entrar', args=[partida.codigo]))
...
def entrar(request, codigo):
    ...
    if partida.status != Partida.Status.AGUARDANDO:
        return render(request, 'cacapalavras/entrar.html', {
            'partida': partida,
    ...
    return render(request, 'cacapalavras/entrar.html', {'partida': partida})
...
def _criar_rodada(partida, fase, sequencia, nivel, qtd_palavras=None, duracao=None,
    ...
    for participante in alvo:
        grid, palavras = gerador.gerar_quadro(nivel, qtd_palavras=qtd_palavras)
...
def _finalizar_rodada(rodada):
    ...
    for quadro in rodada.quadros.select_related('participante__jogador'):
        completou = quadro.completou()
        encontradas = quadro.contar_encontradas()
        if completou and quadro.terminou_em is None:
            quadro.terminou_em = timezone.now()
        # Achar a única palavra de uma rodada de desempate não conta como
        # "completar o nível" — só pontos de conclusão normais valem o bônus.
        if completou and not rodada.desempate:
        ...
        info.append({
            ...
            'completou': completou,
            'terminou_em': quadro.terminou_em,
        })

    seed = f'{partida.codigo}-{rodada.fase}-{rodada.sequencia}'
    classificacao = gerador.ordenar_por_desempenho(info, seed=seed)
    pior_valor = classificacao[-1]['encontradas']
    piores = [item for item in classificacao if item['encontradas'] == pior_valor]
    ...
    if restantes <= 1:
        ...
        if campeao is not None:
            Trofeu.objects.get_or_create(
                ...
                modalidade=Trofeu.Modalidade.CACAPALAVRAS,
            )
    else:
...
def jogo(request, codigo):
    ...
    if participante is None:
        if _eh_host(request, partida):
            return redirect('cacapalavras_painel_host', codigo=codigo)
        return redirect('cacapalavras_entrar', codigo=codigo)

    rodada = partida.rodada_atual()
    if rodada is None:
        return redirect('cacapalavras_lobby', codigo=codigo)
    ...
    if partida.status == Partida.Status.FINALIZADA:
        return redirect('cacapalavras_campeao', codigo=codigo)

    if participante.eliminado:
        return redirect('cacapalavras_resultado', codigo=codigo)
    ...
    if quadro is None:
        ...
        return render(request, 'cacapalavras/aguardando.html', {'partida': partida})

    if rodada.encerrada:
        return redirect('cacapalavras_resultado', codigo=codigo)

    lista_para_exibir = [
        {'palavra': item['exibida'], 'encontrada': item['encontrada']}
        for item in quadro.lista_palavras
    ]

    return render(request, 'cacapalavras/jogo.html', {
        'partida': partida,
        'rodada': rodada,
        'quadro': quadro,
        'grid': quadro.grid,
        'lista_palavras': lista_para_exibir,
        'tamanho_grid': len(quadro.grid),
...
def estado_jogo(request, codigo):
    ...
    for outro in rodada.quadros.select_related('participante__jogador').exclude(participante=participante):
        encontradas = outro.contar_encontradas()
        oponentes.append({
            'apelido': outro.participante.jogador.apelido,
            'progresso': encontradas,
            'total': len(outro.lista_palavras),
...
def validar_palavra(request, codigo):
    ...
    try:
        ...
        celulas = [tuple(c) for c in dados.get('celulas', [])]
    except (json.JSONDecodeError, TypeError, ValueError):
        return JsonResponse({'tipo': 'invalida'})

    quadro = get_object_or_404(QuadroJogador, rodada=rodada, participante=participante)
    resultado = gerador.validar_selecao(
        celulas, quadro.grid, quadro.lista_palavras, quadro.bonus_encontrados,
    )

    if resultado['tipo'] == 'bonus':
        quadro.pontos_rodada += 5

    quadro.save()

    linha_reta = resultado.pop('linha_reta', False)
    palavra_formada = resultado.pop('palavra_formada', None)
    if resultado['tipo'] == 'invalida' and linha_reta:
        PalavraParaAvaliacao.objects.get_or_create(
            rodada=rodada,
            participante=participante,
            palavra_ascii=palavra_formada,
            defaults={'partida': partida, 'celulas': resultado['celulas']},
        )
    ...
    resultado['pontos_rodada'] = quadro.pontos_rodada
...
def resultado_fase(request, codigo):
    ...
    quadros = sorted(
        ...
        key=lambda q: -q.contar_encontradas(),
    )
    return render(request, 'cacapalavras/resultado_fase.html', {
        'partida': partida,
...
def campeao(request, codigo):
    ...
    return render(request, 'cacapalavras/campeao.html', {
        'partida': partida,
...
def painel_host(request, codigo):
    ...
    return render(request, 'cacapalavras/painel_host.html', {'partida': partida})
...
def estado_host(request, codigo):
    ...
    for p in partida.participantes.select_related('jogador').order_by('-pontos_totais'):
        ...
        participantes.append({
            ...
            'encontradas': quadro.contar_encontradas() if quadro else None,
            'total_palavras': len(quadro.lista_palavras) if quadro else None,
        })
